...
const { Config, BaseError, Mutex } = require("uu_appg01_core-utils");
...
const WARNINGS = {
  ...
  logRecordPrivilegedList: {
    unsupportedKeys: {
      code: `${Errors.LogRecordPrivilegedList.UC_CODE}unsupportedKeys`,
    },
  },
  privilegedDelete: {
    unsupportedKeys: {
      code: `${Errors.PrivilegedDelete.UC_CODE}unsupportedKeys`,
    },
  },
  configRecordCreate: {
    configRecordWasCreated: {
      code: `${Errors.CreateAuditLogConfig.UC_CODE}unsupportedKeys`,
      message: "AuditLogConfig was already created.",
    }
  }
};
...
const SUPPORTED_SCHEMA_VERSIONS = ["0.9.0", "1.0.0", "1.1.0"];
const DEFAULT_SCHEMA_VERSION = SUPPORTED_SCHEMA_VERSIONS[0];
...
class SysAuditLogAbl {
  ...
  async createAuditLogRecord(dtoIn) {
    ...
    if (this.appLogStoreUri) {
      ...
      if (this.appLogStoreNotTransferedCount >= this.appLogStoreTransferAfterCount) {
        // auditLog count condition filled
        // HDS 6 Transfers records to the uuAppLogStore - calls sys/logRecord/transfer command
        this.mutex.runOneAtTime(this.transfer.bind(this), uuAppErrorMap, false, false).catch(e => this.logger.error(`Failed to call sys/logRecord/transfer command.`, e));
      } else if (this._checkTimeTransferCondition(this.appLogStoreLastTransferDate)) {
        // check time condition
        this.mutex.runOneAtTime(this.transfer.bind(this), uuAppErrorMap, true, false).catch(e => this.logger.error(`Failed to call sys/logRecord/transfer command.`, e));
      }
    }
    ...
    if (this.recordPruneEnabled) {
      ...
      if (this.recordsWritten >= this.recordLimitCheckAfter) {
        ...
        this.mutex.runOneAtTime(this._pruneAuditLogRecords.bind(this)).catch(e => this.logger.error("Pruning of old uuAuditLog records failed.", e));
      }
    }
   ...
   * @return {object} filled dtoOut object
   */
  async transfer(uuAppErrorMap = {}, checkLatestTransferDate = false, preserveActor = true) {
    let dtoOut = {};
    ...
    if (!this.appLogStoreUri) {
      throw new Errors.LogRecordTransfer.AppLogStoreUriIsNotSet(
        { uuAppErrorMap },
        { appLogStoreUri: this.appLogStoreUri }
      );
    }
    ...
    try {
      ...
      asidAuditLogConfig = await this.configDao.updateById(
        asidAuditLogConfig.id,
        { latestTransferState: TRANSFER_STATES.running },
        lock
      );
    } catch (e) {
      if (e instanceof ObjectLocked) {
        ...
        throw new Errors.LogRecordTransfer.SysAuditLogConfigDaoUpdateFailed(
          { uuAppErrorMap },
          { asid: this.asid, id: asidAuditLogConfig.id },
          e
        );
      }
    }
    ...
    if (!transferAlreadyStarted) {
      this._transferLogsToLogStore(this.appLogStoreUri, lock, asidAuditLogConfig, preserveActor, uuAppErrorMap);
    }
    // HDS 5 Returns properly filled dtoOut.
    dtoOut.uuAppErrorMap = uuAppErrorMap;
    return dtoOut;
  ...
  async _transferLogsToLogStore(logStoreUri, lock, asidAuditLogConfig = {}, preserveActor = true, uuAppErrorMap = {}) {
    ...
    try {
      ...
      while (true) {
        ...
        const res = await LogStoreClient.importLogRecords(logStoreUri, logsToTransfer.itemList, preserveActor);
      ...
      try {
        await this.configDao.updateById(
          asidAuditLogConfig.id,
          { latestTransferState: TRANSFER_STATES.completed, uuAppErrorMap: "{}" },
          lock
        );
      } catch (e) {
        throw new Errors.LogRecordTransfer.SysAuditLogConfigDaoUpdateFailed(
      ...
      const error = new Errors.LogRecordTransfer.TransferRecordsToAppLogStoreFailed(
        { uuAppErrorMap },
        { asid: this.asid, appLogStoreUri: this.appLogStoreUri },
        e
      );
      ...
      await this.configDao.updateById(
        asidAuditLogConfig.id,
        {
          latestTransferState: TRANSFER_STATES.completedWithError,
          uuAppErrorMap: JSON.stringify(dtoOut.uuAppErrorMap),
        },
        lock
      );
    } finally {
  ...
  async _pruneAuditLogRecords() {
    ...
    if (this.prunePolicy === PRUNE_POLICY_TRANSFERRED_LOGS_ONLY) {
      ...
      if (!sysAuditLogConfig || !sysAuditLogConfig.latestTransferredLogId) {
        this.logger.error(
          `Prune policy set to transferredLogsOnly, but the log transfer never started. uuAuditLog records prune wont be triggered. asid ${this.asid}`
        );
        return false;
      }
