...
class TransformParametersHandler {
  ...
  async invoke(request, options = null) {
          ...
          (request.body instanceof FormData ||
            ...
            (typeof request.body == "object" &&
              Object.keys(request.body).some(
                it => request.body[it] instanceof Buffer || request.body[it] instanceof Readable
              )));
    ...
    if (opts.getBoolean("transformParameters") !== false && request.body != null) {
      let contentHandler = contentTransformers[getMimeMainType(contentType)];
      try {
        if (contentHandler && typeof request.body !== "string") {
          request.body = contentHandler.serialize(request.body);
        }
      } catch (e) {
        throw new BaseError("Error while serializing request data.", e);
      }
    }
    ...
    if (opts.getBoolean("transformResponse") !== false && !respContentDisposition) {
      ...
      let contentHandler = contentTransformers[getMimeMainType(respContentType)];
      try {
        if (response.status === 204) {
          response.data = null;
        } else if (contentHandler) {
          response.data = contentHandler.deserialize(response.data);
        }
      } catch (e) {
