...
const { Json, Digest } = require("uu_appg01_core-utils");
...
class Request {
  ...
  isResourceCached(opts = {}) {
    ...
    const isEtagMatch = () => {
      let origin = this.getHeaders()["origin"] || "undefined";
      let expectedEtag = `${opts.etag}.${Digest.sha1(origin)}`;
      return !!(opts.etag && etagHeader.split(",").some(tag => tag.trim().replace(/^(W\/)?"|"$/g, "") == expectedEtag));
    };
