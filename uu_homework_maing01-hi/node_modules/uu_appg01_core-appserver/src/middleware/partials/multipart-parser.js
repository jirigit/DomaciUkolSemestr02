...
function multipartParser(req, res, next) {
  ...
  busboy.on("field", (fieldname, val, info) => {
    ...
    if (info.nameTruncated) return done(new BadRequest("Exceeded size limit for a fieldname."));
    if (info.valueTruncated) return done(new BadRequest(`Exceeded size limit for value in field: ${fieldname}`));
  ...
  busboy.on("file", (fieldname, file, info) => {
    ...
    stream.filename = info.filename;
    stream.contentType = info.mimeType;

    logger.debug(
      "File [" + fieldname + "]: filename: " + info.filename + ", encoding: " + info.encoding + ", mimetype: " + info.mimeType
    );
