...
class MappingsParser {
  ...
  _loadRoutes(mappings) {
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      let productRoute = this._parseProductRoute(product);
  ...
  /**
   * Parses the product specified in the mappings.json file.
   * Restricts product route parameters that do not contain a custom regex with a regex that doesn't allow matching on '-' in addition to the standard express '/'.
   * This prevents suffixes like '-spp' from being matched as a part of the last product route parameter.
   * e.g. {vendor}-{app}-{subapp} productRoute will not match uu-myapp-maing01-async this way - the spp suffix needs to be captured explicitly.
   * This fix is only applied for product routes that contain multiple parameters
   * @param {string} product - product route specified in mappings.json
   * @returns {string} - part of useCase path for regex specific to product
   */
  _parseProductRoute(product) {
    const oneParamProduct = /^{[^{}]*}$/.test(product);
    if (oneParamProduct) {
