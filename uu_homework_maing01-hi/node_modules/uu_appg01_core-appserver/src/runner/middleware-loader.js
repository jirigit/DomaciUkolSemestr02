...
class MiddlewareLoader {
  ...
  load() {
    ...
    for (let i = 0; i < middleware.length; i++) {
      ...
      if (mwInstance.mw) {
        this._logger.info(`Middleware ${mwInstance.constructor.name} uses deprecated function ".mw" to load. Use ".pre" or ".post"`);
      }
      ...
      if (p) {
        ...
        if (Array.isArray(p)) {
          ...
          throw new BaseError(`Unsupported declaration type of middleware profiles in ${mwInstance.constructor.name}. Use Array or String.`);
        }
        if (!match) {
      ...
      if (mwInstance.pre) {
        if (typeof mwInstance.pre === "function" && mwInstance.pre.length !== 3) {
          throw new BaseError(`Unsupported number of parameters in ${mwInstance.constructor.name} pre function. Use (req, res, next).`);
        }
        preControllerMiddleware.push(this._uncaughtErrorWrapper(mwInstance, mwInstance.pre));
      }

      if (mwInstance.post) {
        if (typeof mwInstance.post === "function" && mwInstance.post.length !== 3) {
          throw new BaseError(`Unsupported number of parameters in ${mwInstance.constructor.name} post function. Use (req, res, next).`);
        }
        postControllerMiddleware.unshift(this._uncaughtErrorWrapper(mwInstance, mwInstance.post));
      }

      if (mwInstance.onError) {
        if (typeof mwInstance.onError === "function" && mwInstance.onError.length !== 4) {
          throw new BaseError(`Unsupported number of parameters in ${mwInstance.constructor.name} onError function. Use (err, req, res, next).`);
        }
        postControllerMiddleware.unshift(this._uncaughtErrorWrapper(mwInstance, mwInstance.onError));
      }

      if (mwInstance.ensure) {
        if (typeof mwInstance.ensure === "function" && mwInstance.ensure.length !== 2) {
          throw new BaseError(`Unsupported number of parameters in ${mwInstance.constructor.name} ensure function. Use (req, res).`);
        }
  ...
  _uncaughtErrorWrapper(instance, mw) {
    if (Array.isArray(mw)) {
      return mw;
    } else {
      mw = this._autoBind(instance, mw);
      if (mw.length === 3) { // pre or post function
        return async (req, res, next) => {
          try {
            await mw(req, res, next);
          } catch (e) {
            this._logger.warn(`Wrong implementation of ${instance.constructor.name} middleware. Middleware throws unhandled error instead of calling "next(error)".`, e);
            next(e);
          }
        }
  ...
  _autoBind(instance, mw) {
    if (Array.isArray(mw)) {
      ...
      return mw.bind(instance);
    } else {
      throw new BaseError(`Unsupported type "${typeof mw}" of ${instance.constructor} middleware. Use function or array of functions.`);
    }
  }
