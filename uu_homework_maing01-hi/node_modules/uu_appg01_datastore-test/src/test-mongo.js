...
class TestMongo {
  ...
  isRunning() {
    return !!(this._mongod && this._mongod.instanceInfo);
  }
  ...
  getPort() {
    ...
    return this._mongod.instanceInfo.port;
  }
  ...
  getDbPath() {
    ...
    return this._mongod.instanceInfo.dbPath;
  }
  ...
  async start() {
    await this._mongod.start();
    this._connectionString = this._mongod.getUri().replace(/[^/]*$/, this._dbName);
    if (this._replSet) {
  ...
  async _getClient(db, mongoClientOpts) {
    let connectionString = this.getConnectionString(db);
    let client = this._clients[db];
    if (!client) {
      client = await MongoClient.connect(connectionString, { ...MONGO_CLIENT_OPTS, ...mongoClientOpts });
