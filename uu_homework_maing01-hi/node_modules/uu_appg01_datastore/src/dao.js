...
/**
 * @typedef {{pageIndex: number, pageSize: number, total: number}} PageInfo
 */
class UuDataStoreDao {
   ...
   * Returns an object with itemList and pageInfo, where the itemList is an array of objects and pageInfo is an object with pageIndex, pageSize and total.
   * If the pageInfo.total attribute is set to -1, then the total number of objects is not calculated and instead the resulting pageInfo.total is set to -1.
   * @async
   * @param {object} filter
   * @param {Partial<PageInfo>} pageInfo
   * @param {(string|array|object)} sort
   * @param {Record<string, any>} projection
   * @returns {Promise<{itemList: object[], pageInfo: PageInfo}>} array of objects or empty array
   */
  async find(filter, pageInfo = {}, sort = {}, projection = {}) {
    ...
    try {
      const totalCount = pageInfo.total === -1 ? -1 : await this.count(filter);
    ...
    try {
      ...
      if (
        ...
        !(
          ...
          e.cause.message.match(/topology is closed/i)
        )
      ) {
        throw e;
      }
