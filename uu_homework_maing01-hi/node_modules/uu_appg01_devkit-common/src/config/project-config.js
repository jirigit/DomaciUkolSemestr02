    ...
    if (this._libraryList === undefined) {
      ...
      if (appJsonPath) {
        ...
        this._libraryList = keys
          .map((name) => {
            if (name === ".") return;
            if ((appJson[name] && appJson[name].version) || typeof appJson[name] !== "object") return; // same-release-cycle modules must have same version (i.e. from root of uuapp.json)
            ...
            return { name, path: modulePath, type, ...root, ...appJson[name], pkg: pkg.get() };
          })
          .filter(Boolean);
