...
module.exports = async function(opts) {
  ...
  try {
    ...
    let namespaceAllPartsWithGeneration = [pack.vendor, ...pack.nameNoVendor.replace(/-.*/, "").split("_")]; // uu_demoapp_maing01-hi => [uu, demoapp, maing01]
    let context = {
      ...opts,
      vendor: pack.vendor,
      ...
      namespaceAllParts,
      namespaceAllPartsWithGeneration,
      ...
      secretStoreMasterKey1: generateMasterKey(64),
      secretStoreMasterKey2: generateMasterKey(64)
    };

    let isLib = type.endsWith("lib") || type.endsWith("devkit");
    ...
    if (type === "uu5-lib" || type === "lib") {
      let exampleConfigTplPath = path.join(templatesDir, "__example-config.js");
      let extras = [
        {
          relFile: path.basename(exampleConfigTplPath),
          relDestFile: path.join("..", "example-config.js"),
          fullPath: exampleConfigTplPath,
        },
      ];
      usedItems = usedItems.concat(extras.filter((it) => !fs.existsSync(it.relDestFile)));
    }
    ...
    if (isLib) {
      ...
      parentDirItems.push({
        relFile: ".pnpmfile.cjs",
        fullPath: path.resolve(templatesDir, ".pnpmfile.cjs"),
        relDestFile: path.join("..", ".pnpmfile.cjs"),
      });
    ...
    if (devkit.development.appDeploymentUri === undefined && type !== "nodejs-lib" && type !== "nodejs-devkit") {
    ...
    if (type === "nodejs-app" || type === "nodejs-service") {
      ...
      // Create folder for metamodel
      fs.mkdirSync(path.join(".", "app", "config", "metamodel", "uu-businessterritory-maing01"), { recursive: true });
...
function getFileFilter(type) {
  ...
  else if (type === "nodejs-lib" || type === "nodejs-devkit" || type === "uu5-app") {
    excludedFiles = [
...
function generateMasterKey(length, chars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz~!@#$%^&*()_+-={}[]:;<>?,./|") {
  let result = '';
