...
class AppClient {
  constructor(token, opts = {}) {
    this.token = token;
    this.os8BaseUri = opts.uuOs8ServerUri || ConfigHelper.getConfigParam('uu_os8_server_uri') || DEFAULT_OS8_BASE_URI;
    this.c3BaseUri = opts.uuC3ServerUri || ConfigHelper.getConfigParam('uu_c3_server_uri') || DEFAULT_C3_BASE_URI;
    this.logstoreBaseUri = opts.uuLogStoreServerUri || ConfigHelper.getConfigParam('uu_logstore_server_uri') || DEFAULT_LOGSTORE_BASE_URI;
  }
  ...
  async exchange(url, method, params, headers = DEFAULT_HEADERS, tryNumber = 0, gotOpts = {}) {
    ...
    try {
      result = await Got(url, {
        headers: headers,
        method: method,
        ...(/^get$/i.test(method) || params == null ? undefined : { body: params }),
        ...gotOpts
      });
    } catch (e) {
      ...
      if (e.response && (e.response.statusCode == 401 || e.response.body.toString().indexOf("UU.OS.APPLICATION/E003-NOT_AUTHENTICATED") > 0)) {
