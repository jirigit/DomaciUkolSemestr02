...
function _saveLogs(logs, argv) {
  ...
  if (argv._=="logstore-export" || argv._=="logstoreExport") {var logEnv = (argv.appDeploymentUri ? argv.appDeploymentUri : argv.asid)
    .toLowerCase()
    .replace(/\[[a-zA-Z0-9]*\]/g, "")
    .replace(/\-|\_|\:/g, ".");
...
function _saveFormattedLogs(rawLogPath, logs) {
  ...
  const itemList = logs.itemList ? "itemList" : "pageEntries";
  logs[itemList].forEach(rec => {
