...
const Errors = require("../api/errors/<%= context.projectClassFile%>-error.js");
...
const logger = LoggerFactory.get("<%= context.projectClassName%>Abl");

class <%= context.projectClassName%>Abl {
  constructor() {
  ...
  async init(uri, dtoIn, session) {
    ...
    // HDS 2
    const schemas = ["<%=  context.projectClassName.charAt(0).toLowerCase() + context.projectClassName.substr(1) %>"];
    ...
    if (dtoIn.uuBtLocationUri) {
      ...
      const createAwscDtoIn = {
        name: "<%= context.namespaceApp%>",
        typeCode: "<%= context.insomniaContextPath%>",
        location: location,
...
module.exports = new <%= context.projectClassName%>Abl();
