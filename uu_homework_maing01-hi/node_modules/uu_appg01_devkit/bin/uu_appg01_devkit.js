...
if (!process.env.UU_APPG01_DEVKIT_NODEVC) {
  ...
  if (semver.compare(process.version.replace(/^v/, ""), NODEJS_VERSION) < 0) {
    console.warn(
      require("chalk").yellow("WARNING") +
        ` Old NodeJS version ${process.version} detected. Use newest LTS version, at least ${NODEJS_VERSION.replace(
          /(\.0)+$/,
          ".x"
        )}.`
    );
...
yargs
  ...
  .command(
    "vulnerabilityScan",
    "Scans all project dependencies and checks them for known vulnerabilities.",
    {},
    getScript("vulnerabilityScan")
  )
  ...
  .command("uuAppBoxCopy [tokenFile]", "Copy uuAppBox from UU to Nexus.", {}, getScript("uuAppBoxCopy"))
  .command("uuAppDevKitUpgrade", "Upgrades project to the current devkit version.", {}, getScript("uuAppDevKitUpgrade"))
  ...
  .command(
    "uuCloudSwap [tokenFile]",
    "Swaps appdeployment between the PRODUCTION and BETA slots.",
    {},
    getScript("uuCloudSwap")
  )
  .command("uuLogStoreExport [tokenFile]", "Export logs of the application.", {}, getScript("uuLogStoreExport"))
  .command(
    "uuCloudLogStoreExport [tokenFile]",
    "Export logs of the application.",
    {},
    getScript("uuCloudLogStoreExport")
  )
  ...
  .command(
    "uuCmdCreateBookit [pageUri]",
    "Generate uuCMD according to it's standardised design",
    {},
    getScript("generateCmdBookit")
  )
  ...
  .command(
    ...
    "Merge profiles.json of all libraries with application profiles.json and save them to the /target folder.",
    {},
    getScript("generateProfilesJson")
  )
  .command(
    "generateMappingsJson",
    "Merge mappings.json of all libraries with application mappings.json and save them to the /target folder.",
    {},
    getScript("generateMappingsJson")
  )
...
function getScript(scriptName, templateType = null) {
  return async function (argv) {
    try {
      ...
      // show warning if project was upgraded to newer version of devkit but currently installed devkit is older
      // (but don't show it if running uuAppDevKitUpgrade script)
      if (!process.env.UU_APPG01_DEVKIT_SPECVC) {
        process.env.UU_APPG01_DEVKIT_SPECVC = "1";
        if (scriptName !== "uuAppDevKitUpgrade") {
          let { specVersion } = helpers.getTemplateInfo();
          let myVersion = require("../package.json").version;
          if (semver.valid(specVersion) && semver.compare(specVersion, myVersion) > 0) {
            console.warn(
              require("chalk").yellow("WARNING") +
                ` This project was already upgraded to uu_appg01_devkit ${specVersion}, you have ${myVersion}. Install newer devkit to prevent incompatibility issues.`
            );
          }
        }
      }
      ...
      // kill child processes - if there are any then it means that we're either handling
      // uncaught exception which will end this process (and we want to kill subprocesses
      // so that they dno't keep ports taken, ...), or the devkit implementation is not waiting
      // for children correctly
      let childPids = await require("pidtree")(process.pid);
      childPids.forEach((childPid) => {
        try {
          process.kill(childPid, "SIGTERM");
        } catch (e) {
          // process might have ended already in the meantime
