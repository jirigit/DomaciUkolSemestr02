...
module.exports = function (pkg, buildDir, useTranspilation = true, workspaceLibraries = []) {
  let useUu5g05 =
  ...
  if (packs && packs.length) {
    ...
    for (let k in submodules)
      moduleNameMapper["^(" + StringUtil.regexpQuote(k) + ")$"] = "<rootDir>/src/" + submodules[k].replace(/\\/g, "/");
  }
  if (useUu5g04) {
    ...
    if (pkg.name !== "uu_plus4u5g01") {
      moduleNameMapper["^uu_plus4u5g01[-/]([^/?!]+)"] = "uu_plus4u5g01/$1.js";
    }
  }
  ...
  let result = {
    ...
    transform: useTranspilation
      ? {
          "^.+\\.(js|jsx|mjs)$": [path.join(__dirname, "jest-babel-transform.js"), { workspaceLibraries }],
          "\\.(png|svg|gif|jpg|jpeg|ico)$": path.join(__dirname, "jest-misc", "image-transform.js"),
        }
      : {},
  ...
  if (result["testURL"]) {
    let url = result["testURL"];
    delete result["testURL"];
    console.warn(
      require("chalk").yellow("WARNING") +
        " Your package.json contains jest.testURL key which is deprecated. Use jest.testEnvironmentOptions.url instead."
    );
    result.testEnvironmentOptions = {
      url,
      ...result.testEnvironmentOptions,
    };
  }
