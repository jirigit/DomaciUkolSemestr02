...
module.exports = class Build {
  ...
  async process() {
    ...
    let { useCdn, useCdnExactVersions } = config;
    ...
    let depProcessor = new DependencyProcessor("package.json", useCdn, useCdnExactVersions);
    if (!config.skipExternalsDiscovery) {
      let externalsForApp = depProcessor.getExternals("app", { printErrors: true });
      if (externalsForApp.errorredExternals.length > 0) process.exit(1);
      config.externals = externalsForApp.config;
    ...
    if (!useCdn && config.externals && config.externals["react-dnd-touch-backend"]) {
      let {
        ...
        localBaseUri,
        main = "Touch.js",
        mainMinified = "Touch.browserified.js",
      } = config.externals["react-dnd-touch-backend"];
        ...
        if (usedBaseUri == null && depBaseUri) {
          if (depBaseUri.match(/^(\/|https?:)/)) {
            ...
            if (srcLibsMap[depName]) {
              depTargetFldName = srcLibsMap[depName];
              ...
              let version = depConfig.detail?.meta?.json?.version;
              if (!version) version = getPackageJson(depName, {})?.version || depConfig.detail?.cdnVersion;
    ...
    let env = {
      ...
      isCdnDisabled: !useCdn,
      isCdnExactVersions: useCdnExactVersions,
    };
