...
class LibraryDescriptorGenerator {
  ...
  generateLibraryDescriptor() {
    ...
    let { sourceUri, imageUri, docUri, namespace, description, componentList, license, singleton, globalNamespace } =
      this.buildConfig.getAll();
    let isForCdn = this.projectType !== "nodejs-lib" && this.projectType !== "nodejs-devkit";
      ...
      libraryList: [
        {
          ...
          ...(isForCdn
            ? {
                sourceUri,
                dependencyMap: DependencyProcessor.getDirectDependencyVersionMap(this.loaderSettings.externals, true),
                ...(singleton != null ? { singleton } : undefined),
                ...(globalNamespace != null ? { globalNamespace } : undefined),
              }
            : null),
        },
      ],
        ...
        for (let depLibFullName in dependencyMap) {
          ...
          if (!depLibUrl) {
            if (depLibParams) {
              ...
              let versionSpecifier = depLibUsedVersion.replace(/^(\d+)\.0\.0$/, "$1.x");
              ...
              dependencyMap[depLibFullName] = versionSpecifier;
            } else {
              throw new Error(
                `Library ${depLibFullName} is declared as external dependency (listed in package.json in "uuBuildSettings.externals")` +
