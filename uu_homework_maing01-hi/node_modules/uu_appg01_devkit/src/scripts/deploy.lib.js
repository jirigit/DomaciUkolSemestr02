...
module.exports = class Deploy {
  ...
  async process() {
    ...
    let { normalizedUuName: name, version, private: priv } = Package.getSingletonSync("package.json").get();

    let templateType = helpers.getTemplateInfo().type;
    if ((!priv || templateType === "nodejs-multi") && npmDeploy === true) {
    ...
    if (templateType === "uu5-lib" || templateType === "lib" || templateType === "nodejs-multi") {
      if (cdnDeploy) {
        ...
        if (token) {
          ...
          const cdnPackBasename = `${name}-${version}-cdn-pack.zip`;
