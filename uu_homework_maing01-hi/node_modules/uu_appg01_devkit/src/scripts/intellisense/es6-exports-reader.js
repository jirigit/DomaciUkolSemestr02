...
class Es6ExportsReader {
  ...
  getExportsFromFile(file, { includeMeta = false } = {}) {
    ...
    if (!exportsWithMeta) {
      ...
      if (fileResolved.match(/\.(jsx?|mjs)$/)) {
        exportsWithMeta = this.getExportsFromJs(jsContent, newDir, { includeMeta: true });
      } else if (fileResolved.match(/\.(json)$/)) {
        exportsWithMeta = { exports: { default: { type: { type: "any" } } }, complete: true, missingSources: [] };
      } else {
        exportsWithMeta = { exports: { default: { type: { type: "string" } } }, complete: true, missingSources: [] };
      }
  ...
  _getNestedExports(dir, moduleImportName, { includeMeta = false } = {}) {
    ...
    if (includeMeta) {
      ...
      if (result.unrecognizedExportStatements) {
        result.unrecognizedExportStatements = result.unrecognizedExportStatements.map((it) =>
          it.file ? it : { ...it, file }
        );
      }
    }
  ...
  _getExportsFromAst(babelAst, moduleImporter) {
    ...
    let unrecognizedExportStatements = [];
    ...
    rootStatements.forEach((it) => {
      if (it.type === "ExportAllDeclaration") {
        if (it.source) {
          ...
          if (nested.unrecognizedExportStatements) {
            unrecognizedExportStatements = unrecognizedExportStatements.concat(nested.unrecognizedExportStatements);
          }
        }
        ...
        if (it.declaration) {
          for (let declaration of it.declaration.declarations || [it.declaration]) {
            let { name, type, elements, properties, loc } = declaration.id || {};
            if (name && type === "Identifier") {
              variables[name] = this._getTypeFromNode(declaration, context) || { type: "any" };
              if (!(name in exports)) exports[name] = { type: variables[name] };
            } else if (type === "ArrayPattern" && elements && elements.every((it) => it.type === "Identifier")) {
              for (let i = 0; i < elements.length; i++) {
                let { name } = elements[i];
                if (name) {
                  variables[name] = { type: "any" };
                  if (!(name in exports)) exports[name] = { type: variables[name] };
                }
              }
            } else if (
              ...
              properties &&
              properties.every(
                (it) => it.type === "ObjectProperty" && it.key?.type === "Identifier" && it.value?.type === "Identifier"
              )
            ) {
              for (let i = 0; i < properties.length; i++) {
                let { key, value } = properties[i];
                if (value.name) {
                  variables[value.name] = { type: "any" };
                  if (!(value.name in exports)) exports[value.name] = { type: variables[value.name] };
                }
              }
            } else {
              exportsComplete = false;
              if (loc?.start) {
                unrecognizedExportStatements.push({ line: loc.start.line, column: loc.start.column });
              }
            }
          }
        } else {
            ...
            if (name && type === "Identifier" && !(name in exports)) {
              ...
              if (moduleImportName) {
                ...
                if (specifier.type === "ExportNamespaceSpecifier") {
                  // e.g. `export * as PropTypes from "./prop-types.js"`
                  let objectFields = {};
                  if (origExports) {
                    for (let [name, ex] of Object.entries(origExports)) {
                      if (name === "default") continue;
                      objectFields[name] = ex.type;
                    }
                  }
                  exports[name] = {
                    type: { type: "namespace", objectFields },
                    sources: [moduleImportName],
                  };
                } else {
    ...
    return { exports, complete: exportsComplete, missingSources: [...missingSources], unrecognizedExportStatements };
  }
