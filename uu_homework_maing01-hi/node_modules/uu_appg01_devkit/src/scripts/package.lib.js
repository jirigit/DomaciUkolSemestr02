...
module.exports = class Package {
  ...
  async process() {
    ...
    let loaderSettings;
    if (type !== "nodejs-lib" && type !== "nodejs-devkit") {
      if (!pkg.uuBuildSettings || !pkg.uuBuildSettings.skipExternalsDiscovery) {
        ...
        loaderSettings = {
          ...pkg.uuBuildSettings,
          externals: new DependencyProcessor("package.json").getExternals("lib").config,
        };
      } else {
    ...
    if (type === "uu5-lib" || type === "lib") {
      ...
      let packForCdn = isLibInWorkspaceMultiLib ? Pack.getSingletonSync("../package.json") : pack;
      let cdnPackName = packForCdn.vendor + "-" + packForCdn.nameNoVendor.replace(/_/g, "-");
  ...
  async _preparePackFiles(pkg, type) {
    // NOTE npm@9 has issue with package.json's "files" array containing "dist*" - it doesn't include the folders
    // dist/ and dist-node/ like npm < 9 did. So we'll warn and prepare the pack in separate folder (which always
    // adds dist* folders).
    let isFilesBugged = type === "lib" && pkg.files?.includes("dist*");
    if (isFilesBugged) {
      console.warn(
        "  " +
          require("chalk").yellow("WARN") +
          " Temporarily fixing `files` array in package.json to not use glob patterns (`dist*`) as npm >= 9.0.0 has issues with that. Perform `npm run uuAppDevKitUpgrade` and commit the changes for permanent fix."
      );
    }
    if (type === "uu5-lib" || type === "lib" || isFilesBugged) {
    ...
    // - typically used in server-side (which usually have only src/ packaged without
    // needing to update anything in final package.json)
  ...
  async _preparePackFilesInSeparateDir(pkg, type) {
    ...
    if (type === "uu5-lib") extraFiles = extraFiles.filter((it) => it !== pkg.main);
    ...
    if (await fs.pathExists(path.join(npmPackDir, "index-browser.js"))) {
      modPkg.browser = "index-browser.js";
    } else {
      delete modPkg.browser;
    }
