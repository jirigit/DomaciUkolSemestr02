...
module.exports = class Build {
  ...
  async process() {
    ...
    let pkg = pack.get();
    let projectName = pkg.normalizedUuName;
    ...
    let libraryPackPath = path.join(paths.buildDir, projectName + "-" + pkg.version + "-library-pack.zip");
    ...
    if (libDescriptorList.length > 0) {
      ...
      let bestMatchLib = libDescriptorList.find(
        (it) => it.libraryList && it.libraryList.some((lib) => lib.name === projectName)
      );
      let libDescriptor = {
      ...
      let libDescriptorPath = path.join(paths.buildDir, projectName + "-" + pkg.version + "-library-descriptor.json");
    ...
    if (cdnPackModules.length > 0) {
      ...
      for (let { fileMap } of cdnPackModules) {
        ...
        unzip(fileMap.cdn, distDir, (fileName) => fileName.split("/").slice(2).join("/"));
      }
      let projectVersion = uuCloudLibraryConfig.getQualifiedVersion();
      let { vendor, nameNoVendor } = pack;
      let cdnPackName = vendor + "-" + nameNoVendor.replace(/_/g, "-").replace(/\/.*/, "");
...
function unzip(zipFile, targetDir, pathMangler) {
  ...
  for (const zipEntry of zipEntries) {
    ...
    if (targetFileName && !zipEntry.isDirectory) {
      fs.ensureDirSync(path.dirname(path.join(targetDir, targetFileName)));
      zip.extractEntryTo(fileName, path.resolve(targetDir), false, true, false, targetFileName);
    }
  }
}
