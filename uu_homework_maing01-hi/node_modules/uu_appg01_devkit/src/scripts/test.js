...
module.exports = class Test {
  ...
  async process() {
    ...
    if (useBrowserSettings) {
      // TODO Do somehow better. Keep "aliases" in sync with src/config/jest-resolver.js :-(.
      // When we're in multi-lib workspace, current project might import something that is actually
      // from other project in the workspace and that thing could use other project's own "aliases"
      // configuration (such as "calls" which should be resolved to other project's src/calls.js,
      // not our project's src/calls.js) => this is now handled in jest-resolver.js
      let devConfig = new ProjectConfig({ ...this.config, mode: "development", hot: false });
      require("./webpack/runner.js").mergeBuildConfig(devConfig);
      require("../config/webpack.config.js").writeAliases(
        devConfig.getAll(),
        path.join(paths.buildDir, "jest-webpack-aliases.json")
      );
      jestConfigArgs.push(paths.buildDir, templateType !== "lib");
      let workspaceLibraryList = this.config.getWorkspaceLibraryList();
      jestConfigArgs.push(workspaceLibraryList.map((it) => it.name));
    }
...
module.exports = ${JSON.stringify(testConfig, null, 2)}`,
    ...
    if (coverage) {
      if (path.resolve(".", helpers.npmScriptBinFile("nyc")).match(/[ ]/)) {
        ...
        console.log(`Generating coverage report to ${path.resolve(".", coverageReportDir)}${path.sep}index.html`);
        child_process.spawnSync(
          [
            path.relative(".", helpers.npmScriptBinFile("nyc")),
            "report",
