...
module.exports = class UuCloudDeploy {
  ...
  constructor(config) {
    this.config = config;
    this.taskName = "uuCloudDeploy"
  }

  async process() {
    let { normalizedUuName: name, version, dependencies, uuBuildSettings } = Package.getSingletonSync("package.json").get();
    ...
    if (templateType === "uu5-lib" || templateType === "lib" || templateType === "nodejs-multi") {
      uuCloudConfig = new UuCloudLibraryConfig(path.resolve("."), name, version, paths.buildDir, this.config, dependencies, uuBuildSettings, this.taskName);
    } else {
      uuCloudConfig = new UuCloudConfig(path.resolve("."), name, version, paths.buildDir, this.config, this.taskName);
    }

    const Task = require("./uu_cloud/task-deploy");
