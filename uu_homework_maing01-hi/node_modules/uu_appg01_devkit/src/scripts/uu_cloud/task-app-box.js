...
class uuAppBoxTask {
  ...
  constructor(cloudConfig) {
    ...
    this.appClient = new AppClient(this.config.token, this.config.appClientOpts);
  }
  ...
  async process() {
    ...
    await this._uploadLibraryDescriptor(appBoxUri);
    await this._uploadLibraryPack(appBoxUri);
    await this._uploadAppPackage(appBoxUri);
    await this._uploadReadme(appBoxUri);
    await this._uploadMetaModels(appBoxUri);
  }
  ...
  async _uploadDescriptor(appBoxUri) {
    if (this.config.cloudDescriptorPath) {
  ...
  async _uploadLibraryDescriptor(appBoxUri) {
    if (this.config.libraryDescriptorPath) {
      console.log(`Uploading library descriptor ${this.config.libraryDescriptorPath}.`);
  ...
  async _uploadLibraryPack(appBoxUri) {
    if (this.config.libraryPackPath) {
      console.log(`Uploading library pack ${this.config.libraryPackPath}.`);
      let code = this.config.libraryPackCode;
      let dataPath = this.config.libraryPackPath;
      await this._uploadAttachment(appBoxUri, code, dataPath);
    }
  }
  ...
  async _uploadMetaModels(appBoxUri) {
    if (!this.config.libraryDescriptorPath) {
      if (!this.config.uuBtMetaModelZip && !this.config.uuMtMetaModelZip) {
        console.log("Warning: Meta model file is missing");
      } else {
        await this._uploadMetaModel(appBoxUri, this.config.uuBtMetaModelCode, this.config.uuBtMetaModelZip);
        await this._uploadMetaModel(appBoxUri, this.config.uuMtMetaModelCode, this.config.uuMtMetaModelZip);
      }
    }
  }
  
  async _uploadMetaModel(appBoxUri, code, dataPath) {
    if (dataPath) {
      console.log(`Uploading meta model ${dataPath}.`);
      await this._uploadAttachment(appBoxUri, code, dataPath);
    }
  }
}
