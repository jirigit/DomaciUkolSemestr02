...
class uuCloudDeployTask {
  ...
  async _runC3Deploy() {
    ...
    if (this.cloudConfig.devkitConfig && this.cloudConfig.devkitConfig.uuCloudBaseUri) {
      params.config.uuCloudBaseUri = this.cloudConfig.devkitConfig.uuCloudBaseUri;
    }
    if (this.cloudConfig.devkitConfig && this.cloudConfig.devkitConfig.uuCloudLogStoreBaseUri) {
      params.config.uuCloudLogStoreBaseUri = this.cloudConfig.devkitConfig.uuCloudLogStoreBaseUri;
    }
  ...
  async _waitForResult(taskUri) {
    ...
    let result;
    try {
      result = await CmdHelper.waitForC3Result(deployCheck);
    } catch (e) {
      throw new Error(
        "The check of deployment status failed. However, deployment is most likely still running and you can check the status in activity that was sent to you in Plus4U." +
          endOfLine +
          e.message
      );
    }
