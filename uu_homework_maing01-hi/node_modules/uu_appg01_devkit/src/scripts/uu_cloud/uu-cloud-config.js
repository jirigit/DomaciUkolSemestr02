...
const BasicAuth = require("uu_appg01_devkit-common/src/scripts/basic-auth.js");
...
const NORTH_GATEWAY_DEV_URI = "https://uuos9-dev.plus4u.net";
const NORTH_GATEWAY_DEV_BETA_URI = "https://uuos9-dev-beta.plus4u.net";
const WEST_GATEWAY_URI = "https://uuapp.plus4u.net";
const WEST_GATEWAY_BETA_URI = "https://uuapp-preview.plus4u.net";
const WEST_GATEWAY_DEV_URI = "https://uuapp-dev.plus4u.net";
const WEST_GATEWAY_DEV_BETA_URI = "https://uuapp-dev-preview.plus4u.net";
...
class UuCloudConfig {
  constructor(projectRoot, projectName, projectVersion, targetDir, buildConfig, taskName) {
    ...
    this.devkitConfigPath = path.resolve(this.projectRoot, paths.uuAppJsonPath);
    let devkitConfigKey = paths.uuAppJsonPath.startsWith("..") ? path.basename(this.projectRoot) : "";
    let devkitConfigFull;
    if (fs.existsSync(this.devkitConfigPath)) {
      devkitConfigFull = JSON.parse(fs.readFileSync(this.devkitConfigPath, "utf-8"));
      if (devkitConfigKey) {
        let specificConfig = devkitConfigFull[devkitConfigKey];
        if (!specificConfig || (!specificConfig.devkit && !specificConfig.version)) devkitConfigKey = "";
      }
    }
    this.devkitConfigPrefixParts = ["$", ...(devkitConfigKey ? [devkitConfigKey] : []), "devkit", this.environment];
    this.devkitConfigPrefix = this.devkitConfigPrefixParts.join(".");
    if (devkitConfigFull) {
      if (devkitConfigKey) {
        let globalConfig = ((devkitConfigFull || {}).devkit || {})[this.environment] || {};
        let projectConfig = ((devkitConfigFull[devkitConfigKey] || {}).devkit || {})[this.environment] || {};
        let mergedConfig = { ...globalConfig, ...projectConfig };
        this.devkitConfig = Object.keys(mergedConfig).length ? mergedConfig : null;
      } else {
        this.devkitConfig = ((devkitConfigFull || {}).devkit || {})[this.environment];
      }
    } else {
      this.devkitConfig = null;
    }
    this.metaModelRootDir = this._getMetaModelDir();
    this.uuBtMetaModelDir = this._getMetaModelDir("uu-businessterritory-maing01");
    this.uuMtMetaModelDir = this._getMetaModelDir("uu-myterritory-maing01");
    ...
    this.oidcCredentialsFile = buildConfig.get("oidcCredentialsFile") || buildConfig.get("tokenFile");
    this.basicCredentialsFile = buildConfig.get("basicCredentialsFile");
    this.oidcToken = null;
  ...
  uuAppBoxConfig() {
    ...
    let appBoxCode = `${appBoxNaming.code}/APPBOX-${baseVersion}`;
    ...
    result.appBoxName = `${appBoxNaming.name} ${baseVersion} uuApp Box`;
    result.appBoxCode = appBoxCode;
    result.appPackageCode = this._getAppPackageCode(
      cloudDescriptor,
      cloudDescriptor ? cloudDescriptor.code : appBoxNaming.code
    );
    result.appPackagePath = this._getAppPackagePath();
    result.cloudDescriptorCode = `${appBoxNaming.code}/UUAPP-DEPLOY`;
    result.cloudDescriptorPath = this._getCloudDescriptorPath();
    result.readmeCode = `${appBoxNaming.code}/README`;
    result.readmePath = this._getReadmePath();
    result.uuBtMetaModelCode = `${appBoxNaming.code}/METAMODEL-UUBT`;
    result.uuMtMetaModelCode = `${appBoxNaming.code}/METAMODEL-UUMT`;
    result.uuBtMetaModelZip = this._getMetaModelZip("metamodel_uu-businessterritory-maing01.zip");
    result.uuMtMetaModelZip = this._getMetaModelZip("metamodel_uu-myterritory-maing01.zip");
    result.token = this._getToken();
    result.appClientOpts = this._getAppClientOpts();
  ...
  uuAppBoxCopyConfig() {
    ...
    result.token = this._getToken();
    result.appClientOpts = this._getAppClientOpts();
    result.appBoxCopyLocationUri = this._getAppBoxCopyLocationUri();
    if (!result.appBoxCopyLocationUri) {
      throw new Error(`Parameter appBoxCopyLocationUri must be set in particular environment in uuapp.json.`);
    }
    result.nexusDirectory = "";

    let match = null;
    // parse https://nexus.uucloud.eu/repository/Apps/directory/ into
    if ((match = result.appBoxCopyLocationUri.match(/^(.*)\/repository\/([^\/]+)\/?(.*)?\/?$/i))) {
      // https://nexus.uucloud.eu
      result.nexusDomain = match[1];
      // Apps
      result.nexusRepository = match[2];
      // directory
      if (match[3]) {
        result.nexusDirectory = match[3] + "/";
        // remove directory from appBoxCopyLocationUri
        result.appBoxCopyLocationUri = result.appBoxCopyLocationUri.slice(0, - (match[3].length + 1));
      }
    }
    result.nexusDirectory += this._appBoxUriToNexusDirectory(result.appBoxUri);
    return result;
  }
  ...
  async uuDeployConfig(includeAsid = true) {
    ...
    if (this.legacyCloudConfig) {
      ...
      if (
        (!this.subAppConfig.resourcePoolUri || this.subAppConfig.resourcePoolUri === VALUE_PLACEHOLDER) &&
        (!this.devkitConfig ||
          !this.devkitConfig.resourcePoolUri ||
          this.devkitConfig.resourcePoolUri === VALUE_PLACEHOLDER)
      ) {
        throw new Error(
          `Parameter $.uuSubApp.resourcePoolUri must be set (in ${this.subAppDescriptorPath}) or ${this.devkitConfigPrefix}.resourcePoolUri must be set (in ${this.devkitConfigPath}).`
        );
      }
      ...
      result.resourcePoolUri =
        (this.subAppConfig.resourcePoolUri !== VALUE_PLACEHOLDER ? this.subAppConfig.resourcePoolUri : null) ||
        (this.devkitConfig && this.devkitConfig.resourcePoolUri);
      if (includeAsid) {
    ...
    result.appBoxUri = this._getAppBoxUri();
    if (this.devkitConfig && this.devkitConfig.appBoxCopyLocationUri) {
      result.appBoxUri = this._appBoxUriToNexusUriOfCloudDescriptor(result.appBoxUri);
    }
    result.token = this._getToken();
    result.appClientOpts = this._getAppClientOpts();
  ...
  generateCloudDescriptor() {
    ...
    if (legacyDescriptorPath) {
      ...
      let { name: projectName, code: projectCode } = this._getAppBoxNaming();
      cloudDescriptor = {};
  ...
  updateUuSubAppDescriptor() {
    ...
    this.cloudDescriptorPath = this._getCloudDescriptorPath() || this._getLegacyCloudDescriptorPath(this.environment);
    let cloudDescriptor = this._getCloudDescriptor();
    let appPackageCode = this._getAppPackageCode(cloudDescriptor, cloudDescriptor.code);
  ...
  storeAppDeploymentUri(uri) {
    if (this.legacyCloudConfig) {
      ...
      if (fs.existsSync(this.devkitConfigPath)) {
        ...
        let parts = this.devkitConfigPrefixParts.slice(1);
        ...
        let devkitConfig = projectConfig[parts[0]];
        if (!devkitConfig) {
          devkitConfig = projectConfig[parts[0]] = {};
        }
        let envConfig = devkitConfig[parts[1]];
        if (!envConfig) {
          envConfig = devkitConfig[parts[1]] = {};
        }
  ...
  _getAuthStrategy(taskName) {
    let strategy = this.devkitConfig && this.devkitConfig[taskName + "AuthStrategy"];
    if (!strategy || strategy.toLowerCase() === "oidc") return "oidc";
    if (strategy.toLowerCase() === "basic") return "basic";
    throw new Error(`Incorrect Authentication Strategy for task ${taskName}. (Use oidc or basic)`);
  }

  _getToken(taskName = null) {
    if (!taskName) {
      taskName = this.taskName;
    }

    if (!this.tokens[taskName]) {
      if (this._getAuthStrategy(taskName) === "basic") {
        this.tokens[taskName] = this._getBasicToken();
      } else {
        this.tokens[taskName] = this._getOidcToken();
      }
    }
    return this.tokens[taskName];
  }

  _getBasicToken() {
    if (!this.basicToken) {
      this.basicToken = new BasicAuth(this.basicCredentialsFile);
    }
  ...
  _getOidcToken() {
    if (!this.oidcToken) {
      this.oidcToken = new OidcToken(this.projectRoot, this.oidcCredentialsFile);
    }
  ...
  _getAppPackagePath() {
    if (!this.appPackagePath) {
      ...
      let files = fs.readdirSync(this.targetDir).filter((fn) => {
        let match = fn.match(new RegExp(`^${this.projectName}-${baseVersion}(|\\..*)\\.zip$`));
        if (match && fn === `${this.projectName}-${this.projectVersion}.zip`) exactMatch = fn;
        return match;
      });
      if (files.length == 0) {
        throw new Error(`App package does not exist at location ${this.targetDir}. Run [package] task to create it.`);
  ...
  _getCloudDescriptorPath() {
    ...
    let exactMatch;
    let files = fs.readdirSync(this.targetDir).filter((fn) => {
      let match = fn.match(new RegExp(`^${this.projectName}-${baseVersion}(|\\..*)-deploy\\.json$`));
      if (match && fn === `${this.projectName}-${this.projectVersion}-deploy.json`) exactMatch = fn;
      return match;
    });
    if (files.length > 0) {
      return path.resolve(this.targetDir, exactMatch || files.sort().pop());
    } else {
      return null;
    }
  }
  ...
  _getAppClientOpts() {
    let opts = {};
    if (this.devkitConfig) {
      opts["uuC3ServerUri"] = this.devkitConfig.uuC3ServerUri || this.devkitConfig.uuCloudBaseUri;
      opts["uuLogStoreServerUri"] = this.devkitConfig.uuCloudLogStoreBaseUri;
    }
    return opts;
  }

  _getAppBoxCopyLocationUri() {
    if (!this.devkitConfig || !this.devkitConfig.appBoxCopyLocationUri) return null;
    // ensure that there is no / at the end
    return this.devkitConfig.appBoxCopyLocationUri.replace(/^(.+?)\/*?$/, "$1");
  }

  _appBoxUriToNexusDirectory(appBoxUri) {
    return appBoxUri.replace(/^ues:/, "");
  }

  _appBoxUriToNexusUriOfCloudDescriptor(appBoxUri) {
    let appBoxNamming = this._getAppBoxNaming();

    if (!/^ues:/.test(appBoxUri)) {
      if (!/\.json$/.test(appBoxUri)) {
        // ensure that there is no / at the end + add /{APPBOXCODE}_UUAPP-DEPLOY.json
        return appBoxUri.replace(/^(.+?)\/*?$/, "$1") + `/${appBoxNamming.code}_UUAPP-DEPLOY.json`
      }
      return appBoxUri;
    }
    let directory = this._appBoxUriToNexusDirectory(appBoxUri);
    let appBoxCopyLocationUri = this._getAppBoxCopyLocationUri();
    return `${appBoxCopyLocationUri}/${directory}/${appBoxNamming.code}_UUAPP-DEPLOY.json`;
  }
  ...
  _getMetaModelDir(subDir) {
    let metaModelDir;
    if (!fs.existsSync((metaModelDir = path.resolve(this.projectRoot, "app/config", subDir ? `metamodel/${subDir}` : "metamodel")))) {
      metaModelDir = null;
    }
    return metaModelDir;
  }

  _getMetaModelZip(fileName) {
    let metaModelZip;
    if (!fs.existsSync((metaModelZip = path.resolve(this.projectRoot, "app/config", fileName)))) {
      metaModelZip = null;
    }
    return metaModelZip;
  }
  ...
  async _resolveAppUrl(awid = null) {
    ...
    let gatewayUri;
    if (this.devkitConfig && this.devkitConfig.uuCloudBaseUri) {
      gatewayUri = this.devkitConfig.uuCloudBaseUri.replace(/\/$/, "");
    } else {
      let productionEnvironment = this.environment === "prod" || this.environment === "production";
      let runtimeStack = this.cloudDescriptor.nodeSets.length > 0 && this.cloudDescriptor.nodeSets[0].runtimeStack;
      let mainGatewayUri;
      let betaGatewayUri;
      if (runtimeStack && runtimeStack.match(/^UU.APPG01\/APPSERVER.*/)) {
        mainGatewayUri = productionEnvironment ? WEST_GATEWAY_URI : WEST_GATEWAY_DEV_URI;
        betaGatewayUri = productionEnvironment ? WEST_GATEWAY_BETA_URI : WEST_GATEWAY_DEV_BETA_URI;
      } else {
        mainGatewayUri = productionEnvironment ? NORTH_GATEWAY_URI : NORTH_GATEWAY_DEV_URI;
        betaGatewayUri = productionEnvironment ? NORTH_GATEWAY_BETA_URI : NORTH_GATEWAY_DEV_BETA_URI;
      }
      gatewayUri = this.subAppConfig.targetSlot === "BETA" ? betaGatewayUri : mainGatewayUri;
    }
  ...
  async _getTid() {
    if (!this.tid) {
      ...
      let response;
      try {
        let appClient = new AppClient(this._getToken(), this._getAppClientOpts());
  ...
  _getAppPackageCode(cloudDescriptor, defaultCodePrefix) {
    if (cloudDescriptor && cloudDescriptor.packs && cloudDescriptor.packs.length > 0) {
      return cloudDescriptor.packs[0].code;
    } else {
      return `${defaultCodePrefix}/CMD-PACK`;
    }
  }
