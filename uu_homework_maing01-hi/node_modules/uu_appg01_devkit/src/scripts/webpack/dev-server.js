...
module.exports = class DevServer {
  ...
  start() {
    ...
    let appAssetsRelativeUrlPathNoVersion = appAssetsRelativeUrlPath
      ? appAssetsRelativeUrlPath.replace(/\d+\.\d+\.\d+[^/]*\/$/, "")
      : ""; // "public/0.1.0/" -> "public/"
    ...
    if (Object.keys(mappings || {}).length > 0) {
      ...
      app.use((req, res, next) => {
        if (
          req.method == "GET" &&
          (!appAssetsRelativeUrlPath ||
            (!req.url.startsWith("/" + appAssetsRelativeUrlPath) &&
              !req.url.startsWith("/" + appAssetsRelativeUrlPathNoVersion))) &&
    ...
    if (appAssetsRelativeUrlPath) {
      ...
      app.use((req, res, next) => {
        ...
        if (origUrl === req.url) req.url = req.url.replace("/" + appAssetsRelativeUrlPathNoVersion, "/");
        next("route");
      });
    }

    devServerMiddlewares = [require("./middlewares/demo-middleware.js")].concat(devServerMiddlewares);
    if (offline) {
...
function customizeCompilerStats(compiler, summarizeWebpackStats) {
  ...
  let initialCompilationEndTime;
  let skipOutput = false;

  // "invalid" <=> invalidated, i.e. a file is changed and Webpack is going to recompile bundles
  compiler.hooks.invalid.tap("uu_appg01_devkit watchlog", (changedFileName, changeTime) => {
    if (!isCompiling) {
      // NOTE CopyWebpackPlugin 9.0.1 has bug where if we use glob such as "demo/**" and the demo folder
      // doesn't exist during startup, it will cause recompilation during startup
      // (node_modules/watchpack/lib/watchpack.js:345 v2.2.0 contains 'removals' with the non-existing
      // demo folder). We don't want to check for existence of those folder when we construct config for
      // the plugin because then the developer would have to restart server if he adds the folder while server is up.
      //   => just omit the recompilation output if it happened within 1s after 1st compilation (it seems
      //      to always be < 300ms anyway)
      if (!changedFileName && Date.now() - initialCompilationEndTime < 1000) {
        skipOutput = true;
        return;
      }
  ...
  compiler.hooks.done.tap("uu_appg01_devkit watchlog", (stats) => {
    ...
    let time;
    if (first) {
      initialCompilationEndTime = Date.now();
      time = initialCompilationEndTime - startTime;
      first = false;
    }
    isCompiling = false;
