...
module.exports = class Middleware {
  ...
  process(request, response, next) {
    ...
    if (request.url.match(/^\/demo\/.*\.html(\?|$)/) && !request.url.match(/oidc-callback/)) {
      ...
      if (fs.existsSync(file) || fs.existsSync((file = path.join(this.outputAbsPath, file)))) {
        ...
        let { sourceUri } = this.config;
        if (sourceUri) {
          let libCdnBaseUriNoVersion = sourceUri.replace(/%s/g, "1.0.0").replace(/\d+\.\d+\.\d+.*/, "");
          modHtml = modHtml.replace(/(['"])([^'"]+)/g, (m, g1, g2) => {
            if (g2.startsWith(libCdnBaseUriNoVersion) && g2.endsWith("/assets/example-config.js")) {
              ...
              let hostname = host ? host.split(":")[0] || undefined : undefined;
              let newUrl = g1 + this._getExampleConfigPath(version, g2, hostname);
              return newUrl + g1 + " data-orig-src=" + g1 + g2;
            }
            return m;
          });
        }
  ...
  _getExampleConfigPath(version, curUrl, hostname) {
    ...
    return (
      resolveToUri(name, version || "local", multiLibPortMap[name], "assets/example-config.js", hostname) || curUrl
    );
  }
};

function resolveToUri(name, version, port, path, hostname = "localhost") {
  if (!version) return null;
  if (version === "as-is") return null;
  if (version === "local") {
    if (port < 0) return null;
    return (port ? "http://" + hostname + ":" + port : "") + "/" + path;
  }
