...
function mergeDefaultConfig(projectConfig, mode, pkg) {
  ...
  if (templateType === "uu5-app") {
    ...
    if (config.basePath == null) result.basePath = isWithoutServer ? "" : "/vendor-app-subapp/0/"; // URL path that serves as root for all content (only on webpack development server)
    if (config.minify == null) result.minify = mode !== "development" && mode !== "test";
    ...
    if (config.packs == null) {
      ...
      if (fs.existsSync(path.join(result.sourcePath || config.sourcePath, "loading.css"))) {
        result.packs.push({
          entryPoints: ["loading.css"],
          outputFile: "loading.css"
        });
      } else if (fs.existsSync(path.join(result.sourcePath || config.sourcePath, "loading.less"))) {
        result.packs.push({
    ...
    if (config.packs == null) {
      if (templateType === "nodejs-lib" || templateType === "nodejs-devkit") {
        result.packs = [];
      } else {
        result.packs = [
          {
