...
function resolvePackageJsonPath(moduleName, baseDir) {
  ...
  try {
    ...
    // fall back to using "resolve-package-path", e.g. for exports-restricted modules (though resolve-package-path
    // seems to not always work correctly under pnpm)
    result = require("resolve-package-path")(moduleName, baseDir || ".");
  }
  return result;
}
...
function npmScriptBinFile(binFileName) {
  ...
  if (!fs.existsSync(partialPath) && !fs.existsSync(path.dirname(partialPath))) {
...
function verifyClass(sourceCode, methodName) {
  ...
  let parsedSourceCode = Parser.parse(sourceCode, { sourceType: "module", ecmaVersion: 2020 });
  let klass;
