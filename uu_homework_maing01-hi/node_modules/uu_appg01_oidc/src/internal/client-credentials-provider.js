...
class ClientCredentialsProvider {
   ...
   * @param awid awid or asid of oidc client.
   * @returns {String} Client ID
   */
  static async getClientId(serviceName, awid = null) {
    let clientId;
    if (this._externalProvider) {
      // Read client id from external provider (if available)
      clientId = await this._externalProvider.getClientId(serviceName, awid);
    }
    if (!clientId) {
      // Read client id from configuration
      let credentials = Config.get(`${CONFIG_PREFIX}${serviceName}${CREDENTIALS_SUFFIX}`);
      if (credentials) {
        if (typeof credentials === "string") {
          clientId = (await this._getFromFile(credentials)).client_id;
        } else {
          clientId = this._getFromHash(credentials).client_id;
        }
      }
    }
    if (!clientId && this._externalProvider) {
      // Read unregistered client od from external provider (if available)
      let clientCredentials = await this._externalProvider.getUnregisteredCredentials(serviceName, awid);
      clientId = clientCredentials && (clientCredentials.client_id || clientCredentials.clientId);
    }
    if (!clientId) {
      // Provide default client id of unregistered client
  ...
  /**
   * Returns true in case clientId matches unregistered client.
   * @param clientId ClientId to be checked.
   * @param serviceName Name of registered authentication service.
   * @param awid awid or asid of oidc client.
   * @returns {boolean} True if clientId matches unregistered client
   */
  static async isUnregisteredClient(clientId, serviceName, awid) {
    let unregisteredClientId;
    if (this._externalProvider) {
      let clientCredentials = await this._externalProvider.getUnregisteredCredentials(serviceName, awid);
      unregisteredClientId = clientCredentials && (clientCredentials.client_id || clientCredentials.clientId);
    }
    if (!unregisteredClientId) {
      unregisteredClientId = Config.get(`${CONFIG_PREFIX}${serviceName}_unregistered_client_id`) || `${UNREGISTERED_CLIENT_ID_PREFIX}${DEFAULT_CLIENT_NAME}`;
    }
    return clientId === unregisteredClientId;
  }
   ...
   */
  static async getClientCredentials(serviceName, awid = null) {
    let credentials;
    if (this._externalProvider) {
      // Read credentials from external provider (if available)
      credentials = await this._externalProvider.getClientCredentials(serviceName, awid);
    }
    if (!credentials) {
    ...
    if (!credentials && this._externalProvider) {
      ...
      credentials = await this._externalProvider.getUnregisteredCredentials(serviceName, awid);
    }
    if (!credentials) {
  ...
  static async _getFromFile(credentials) {
    ...
    return this._getFromHash(parsedFile);
  }
