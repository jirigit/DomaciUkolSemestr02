...
const {Config, Base64, BaseError, Digest, Json} = require("uu_appg01_core-utils");
...
class Session extends CoreSession {
  ...
  constructor(serviceName, oauthClient, supportedAcrValues, claims = null, idToken = null, accessToken = null) {
    ...
    if (attributes) {
      ...
      if (uuIdentity) {
        ...
        let identityData = {name, levelOfAssurance, type, authenticationLevelOfAssurance: identityAcr, uuAppKey, accountType};
      ...
      if (actorIdentityClaim) {
        ...
        let actorIdentityData = {name: actorName, levelOfAssurance: actorLoa, type: actorType, authenticationLevelOfAssurance: actorAcr, uuAppKey: actorUuAppKey, accountType: actorAccountType};
  ...
  async actAs(uuIdentity, scope = null, opts = {}) {
    ...
    let credentials = await ClientCredentialsProvider.getClientCredentials(this._serviceName);
    if (!grantedToken) {
      let subjectToken = Base64.urlSafeEncode(Json.stringify({uuidentity: uuIdentity}));
