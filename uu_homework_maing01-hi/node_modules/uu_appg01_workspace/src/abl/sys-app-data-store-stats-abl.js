...
class SysAppDataStoreStatsAbl {
  ...
  async _calculateDataStoreStatsAsync(stats, lock, uuAppErrorMap) {
    ...
    let storesMaxNoi = this._loadStoresProperty("maxNoi");
    let storesMaxSoi = this._loadStoresProperty("maxSoi");
    ...
    let promises = Array.from(Object.keys(stores)).map(async dataStoreName => {
      ...
      let promises = await Array.from(dataStore.schemaList).map(async schema => {
        ...
        try {
          ...
          throw new Errors.StatsCalculate.CurrentCalculationFailed({ uuAppErrorMap }, { cause: e, collectionName: schema });
        }
        ...
        schemaStats["maxNoi"] = storesMaxNoi[schema];
        schemaStats["maxSoi"] = storesMaxSoi[schema];
  ...
  _loadStoresProperty(property) {
    const { schemas } = PersistenceConfig;

    let storesProperty = {};

    schemas.forEach((key, value) => {
      storesProperty[value] = key[property];
    });

    return storesProperty;
  }
}
