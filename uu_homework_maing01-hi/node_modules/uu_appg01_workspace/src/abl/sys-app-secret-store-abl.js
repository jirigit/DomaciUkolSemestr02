...
const { Config, OptsReader, Finder, LruCache } = require("uu_appg01_core-utils");
...
const PARAM_KEY_CACHE_TTL = "uu_app_secretstore_key_cache_ttl";
const PARAM_KEY_CACHE_SIZE = "uu_app_secretstore_key_cache_size";
const DEFAULT_KEY_CACHE_TTL = 60 * 60;
const DEFAULT_KEY_CACHE_SIZE = 1000;
...
class SysAppSecretStoreAbl {
  ...
  constructor() {
    ...
    this.enableEncryption = this.opts.getBoolean("uu_app_secretstore_enable_encryption", null);
    if (this.enableEncryption == null) {
      if (secretStoreConfig && Config.isProfileActive("production") && !Config.isProfileActive("development", "test")) {
        this.enableEncryption = true; // When secret store config is present, enable encryption by default in production
      } else {
        this.enableEncryption = false;
      }
    }
    if (!this.enableEncryption && Config.isProfileActive("production") && !Config.isProfileActive("development", "test")) {
      this.logger.error(`Unsecure application configuration. Running application with disabled secret store encryption in production environment is not recommended.`);
    }
    ...
    let keyCacheTtl = this.opts.getNumber(PARAM_KEY_CACHE_TTL, DEFAULT_KEY_CACHE_TTL) * 1000;
    let keyCacheSize = this.opts.getNumber(PARAM_KEY_CACHE_SIZE, DEFAULT_KEY_CACHE_SIZE);
    this.keyCache = new LruCache({ maxAge: keyCacheTtl, maxSize: keyCacheSize });
    if (keyCacheTtl > 0) {
      // Workaround to ensure expired keys are actually removed from memory (cache does not automatically remove
      // expired keys, they are removed only when reading them or in case cache is full when adding new items)
      this.keyCacheCleanup = setInterval(() => this.keyCache._prune(0), Math.max(keyCacheTtl / 2, 60000));
    }
  }
  ...
  async getSecret(awid, secretSet, code, wholeEntity = false) {
    ...
    if (secretStoreEntity.masterKeySlot != null) {
      ...
      let derivedKey = this.keyCache.get(salt);
      if (!derivedKey) {
        ...
        derivedKey = await this._createPbkdf2(salt, masterKey1, masterKey2);
        this.keyCache.set(salt, derivedKey);
      }

      // HDS 6 System decrypt data
      let { generatedKey, generatedVector } = derivedKey;
  ...
  async rotateMasterKey(asid, dtoIn, uuAppErrorMap = {}) {
    ...
    let rotatePromise = new Promise((resolve, reject) => {
      ...
      this._runRotateMasterKey(asid, uuSubAppInstance.uuAppSecretStoreRotateStatus, dtoIn.secretCount, uuAppErrorMap).then(
        (result) => {
  ...
  _loadSecretStoreConfig() {
    ...
    if (Fs.existsSync(configPath)) {
      ...
      this.logger.error(`Secret store configuration file ${configPath} not found. It is required to be able to enable secret store encryption.`);
      return null;
    }
  }
    ...
    if (listByNonActiveSlot.itemList && listByNonActiveSlot.itemList.length > 0) {
      ...
      for (let item of listByNonActiveSlot.itemList) {
        ...
        if (item.masterKeySlot != null) {
          let masterKeys = this._resolveMasterKeys(item.masterKeySlot);
          previousMasterKey1 = masterKeys.masterKey1;
          previousMasterKey2 = masterKeys.masterKey2;
  ...
  _resolveMasterKeys(activeSlot = this.activeSlot) {
    ...
    if (!masterKey1 && activeSlot === DEFAULT_ACTIVE_SLOT) { // master key 1 for default slot might be declared without slot in name
