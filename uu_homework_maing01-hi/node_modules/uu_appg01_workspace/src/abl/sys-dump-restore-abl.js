...
const IMPORT_QUEUE_LIMIT = 32000; // 32KB
const RESTORE_QUEUE_LIMIT = 10000000; //10MB
...
class SysDumpRestoreAbl {
  ...
  async import(awid, dtoIn) {
    ...
    await new Promise((resolve, reject) => {
      ...
      let entryPromiseList = []; // indicates whether entries were processed so dtoOut returns after restoring all data
      ...
      unzipStream.on("entry", (entry) => {
        ...
        unzipStream.pause();
        let entryPromise = this._importSchema(
          entry,
          uuAppErrorMap,
          awid,
          appWorkspace,
        ...
        entryPromiseList.push(entryPromise);
        entryPromise.then(
          () => {
            unzipStream.resume()
          },
      ...
      unzipStream.on("end", async () => {
        await Promise.allSettled(entryPromiseList);
        resolve();
      });
    });
  ...
  async restore(dtoIn, isAwidRestoreCall = false) {
    ...
    await new Promise((resolve, reject) => {
      ...
      unzipStream.on("entry", (entry) => {
        unzipStream.pause();
        let entryPromise = this._restoreSchema(
          entry,
          awidList,
          isStrictMode,
          isDroppedDatabase,
  ...
  async _processDataLine(
    ...
    if (checkBinaryForFileId && !isAsid) {
      ...
      if (foundFileId) {
        //HDS 3.7.3
        const dataToWrite = { awid: awidOfObject, data: parsedLine };
        bulkQueue.push(dataToWrite);
        bulkQueueSize.value = bulkQueueSize.value + sizeOf(dataToWrite);
      }
    } else if (awidList.length > 0 && awidList.includes(parsedLine.awid)) {
      //HDS 3.7.3
      const dataToWrite = { awid: parsedLine.awid, data: parsedLine };
      bulkQueue.push(dataToWrite);
      bulkQueueSize.value = bulkQueueSize.value + sizeOf(dataToWrite);
  ...
  async _bulkWrite(
    ...
    //HDS 3.7.4 A9
    if (bulkQueueSize.value > RESTORE_QUEUE_LIMIT || force) {
      let result;
      try {
  ...
  async _saveSchema(filter, dao, archive, awidList) {
    ...
    stringifyStream.write(metadata);
  ...
  async _importSchema(entry, uuAppErrorMap, newAwid, appWorkspace, deletedBinaryRegister, fileIdMapping) {
    ...
    await this._eachLine(entry, async (line, lineNumber) => {
      ...
      if (lineNumber === 0) {
        ...
        if (sizeOf(documentQueue) > IMPORT_QUEUE_LIMIT) {
