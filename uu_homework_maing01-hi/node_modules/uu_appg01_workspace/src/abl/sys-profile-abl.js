...
class SysProfileAbl {
  constructor() {
    ...
    this.RESERVED_PROFILES = ["AsidLicenseOwner", "AwidLicenseOwner", "AwidInitiator", "Public", "Authenticated"];
  ...
  async listAppProfiles(awid, dtoIn, uuAppErrorMap) {
    ...
    let errorClass = ErrorHelper.getErrorClass(awid, InstanceErrors.ProfileList, WorkspaceErrors.ProfileList);
  ...
  async getAppProfile(awid, dtoIn, uuAppErrorMap) {
    ...
    let errorClass = ErrorHelper.getErrorClass(awid, InstanceErrors.ProfileGet, WorkspaceErrors.ProfileGet);
    uuAppErrorMap = ValidationHelper.processValidationResult(
      dtoIn,
      validationResult,
      `${errorClass.UC_CODE}unsupportedKeys`,
  ...
  async setAppProfile(awid, dtoIn, uuAppErrorMap) {
    ...
    let errorClass = ErrorHelper.getErrorClass(awid, InstanceErrors.ProfileSet, WorkspaceErrors.ProfileSet);
    uuAppErrorMap = ValidationHelper.processValidationResult(
      dtoIn,
      validationResult,
      `${errorClass.UC_CODE}unsupportedKeys`,
  ...
  /**
   * Updates profiles of uuSubAppInstance according to a definition in profiles.json.
   * If there is a new version of an application created and its profiles are changed,
   * this command is used to provide this change into already initialized uuSubAppInstance.
   *
   * @param {String} asid
   * @param {Object} dtoIn
   * @param {Object} uuAppErrorMap
  ...
  async updateSubAppInstanceProfiles(asid, dtoIn, uuAppErrorMap) {
    //Performs a logical check of dtoIn.
    let validationResult = this.validator.validate("sysUuSubAppInstanceProfileUpdateDtoIn", dtoIn);
    let errorClass = ErrorHelper.getErrorClass(asid, InstanceErrors.ProfileUpdate, WorkspaceErrors.ProfileUpdate);
    uuAppErrorMap = ValidationHelper.processValidationResult(
      dtoIn,
      validationResult,
      `${errorClass.UC_CODE}unsupportedKeys`,
    ...
    // HDS 1. - System loads a list of profiles (privileged profiles list) for ASID from profiles.json.
    const loadedProfileList = ProfileConfig.getPrivilegedProfiles(asid)
      .sort()
      .filter((profile) => {
        return this.RESERVED_PROFILES.indexOf(profile) === -1;
      });

    // HDS 2. - System loads a list of sysProfile for the ASID using sysProfile DAO listByAwid.
    const asidProfileList = (await this.dao.listByAwid(asid)).itemList.map((profile) => profile.profile).sort();
    // HDS 3 - System compares list of profiles from SysProfiles with profileList from profiles.json.
    if (JSON.stringify(loadedProfileList) !== JSON.stringify(asidProfileList)) {
      // HDS 4. - System creates missing SysProfiles using sysProfile DAO create.
      // HDS 5. - System deletes redundant SysProfiles using sysProfile DAO delete.
      await this._migrateAwid({ awid: asid, awidProfileList: asidProfileList, loadedProfileList, uuAppErrorMap });
    }

    //HDS 6. Returns properly filled dtoOut.
    const { profilesToAdd, profilesToDelete } = this._diffProfiles(asidProfileList, loadedProfileList);
    return { deletedProfiles: profilesToDelete, addedProfiles: profilesToAdd, uuAppErrorMap };
  }
  ...
  /**
   * Updates profiles for specific awid based on roleGroupUriMap
   *
   * @param awid
   * @param roleGroupUriMap
   * @param uuAppErrorMap
   * @param errorCode
   * @returns {Promise<{}>}
   */
  async updateProfilesBasedOnRoleGroupUriMap(awid, roleGroupUriMap = {}, uuAppErrorMap = {}, errorCode = "") {
    let profileList = await this.getAppProfileListByAwid(awid);
    for (let profile of profileList) {
      let roleGroupUri = "ues:[-1]:[-1]";
      if (profile && roleGroupUriMap[profile]) {
        roleGroupUri = roleGroupUriMap[profile];
      }
      await this.setAppProfile(awid, { profile, roleGroupUri }, uuAppErrorMap);
    }
    let specifiedProfiles = Object.keys(roleGroupUriMap);
    let notExistentProfiles = specifiedProfiles.filter((p) => !profileList.includes(p));
    if (notExistentProfiles.length > 0) {
      uuAppErrorMap[`${errorCode}notExistentProfiles`] = {
        type: "warning",
        message:
          "Some of the profiles you specified in the roleGroupUriMap do not exist in the application. They will be ignored.",
        paramMap: { notExistentProfiles },
      };
    }
