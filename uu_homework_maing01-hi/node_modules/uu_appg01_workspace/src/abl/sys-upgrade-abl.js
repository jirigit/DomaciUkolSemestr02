...
class SysUpgradeAbl {
  ...
  async upgrade(dtoIn, uuIdentity) {
    ...
    let upgradePromise = new Promise((resolve, reject) => {
      ...
      this._runUpgrade(dtoIn, asid, connectionString, sysAppConfig, upgradeLock, uuIdentity, uuAppErrorMap).then(
        (result) => {
  ...
  async upgradeCleanup(uuAppErrorMap = {}) {
    ...
    // HDS 3 - 10
    let upgradeCleanupPromise = new Promise((resolve, reject) => {
      let timeoutHandle = setTimeout(() => {
        reject("timeout");
      }, 45000);
      this._runUpgradeCleanup(asid, connectionString, upgradeCleanupLock, uuAppErrorMap).then(
        (result) => {
          clearTimeout(timeoutHandle);
          resolve(result);
        },
