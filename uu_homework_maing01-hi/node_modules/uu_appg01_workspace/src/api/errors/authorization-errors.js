...
const UC_CODE = `${WorkspaceError.APP_WORKSPACE_LIB_ERROR_PREFIX}authorization/`;
...
class SysProfileDaoGetByCodesFailedError extends WorkspaceError {
  constructor() {
    super(...arguments);
    this.message = "get sysProfiles by sysProfile Dao listByProfileList failed.";
...
class InvalidAuthorizationDataError extends WorkspaceError {
  constructor(dtoOut, paramMap = {}, status = 500) {
    super(dtoOut, paramMap);
    this.status = status;
    this.message = "Authorization data are not valid.";
    this.code = `${UC_CODE}invalidAuthorizationData`;
  }
}
