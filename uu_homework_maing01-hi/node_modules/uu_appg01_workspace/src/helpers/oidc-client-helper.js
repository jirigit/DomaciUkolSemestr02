...
class OidcClientHelper {
  static async initOidcClient(awid, uuAppErrorMap, logger, uuAsidEeCredentialsInitToken = null) {
    ...
    if (!initOidcClient) {
      logger.debug(
        `Implicit initialization of OidcClient is disabled. Use configuration parameter ${PARAM_INIT_OIDC_CLIENT} to enable it.`
      );
      return;
    }
    ...
    if (!UseCaseContext.isAvailable()) {
      logger.debug(
        "Unable to invoke implicit initialization of OidcClient, initAppWorkspace was not called within scope of request."
      );
      return;
    }
    ...
    try {
      ...
      if (uuAsidEeCredentialsInitToken) {
        result = await new AppClient(opts).post(INIT_OIDC_CLIENT_ENDPOINT, { awid, credentialsInitToken: uuAsidEeCredentialsInitToken });
      } else {
