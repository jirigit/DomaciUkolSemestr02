...
const SYS_STATES_TO_BE_INITED = ["created", "assigned"];
...
class AppWorkspaceInitMiddleware {
  ...
  async post(req, res, next) {
    ...
    if (uri.getAwid() !== this.asid && uri.getUseCase() === "sys/uuAppWorkspace/init") {
      ...
      let workspace = await AppWorkspaceComponent.get(awid);
      if (SYS_STATES_TO_BE_INITED.includes(workspace.sysState)) {
        await AppWorkspaceComponent.setActiveSysState(awid);
        //TODO baseregistry
      }
    }
    next();
