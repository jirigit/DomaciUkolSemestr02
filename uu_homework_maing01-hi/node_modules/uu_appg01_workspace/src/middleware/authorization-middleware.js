...
const PARAM_META_MODEL_VERSION = "uu_app_meta_model_version";
...
class AuthorizationMiddleware {
  constructor() {
    ...
    this._uuAppMetamodelVersion = Config.get(PARAM_META_MODEL_VERSION);
  }
    ...
    if (this._unprotectedRegexp.test(req.path)
      ...
      try {
        await Perflog.measureSection("UU_APP_WORKSPACE_AUTHORIZE", async section => {
          try {
            authzResult = await WorkspaceAuthorizationService.authorize(session, authorizationUri, { uuAppMetamodelVersion: this._uuAppMetamodelVersion });
          } catch (e) {
